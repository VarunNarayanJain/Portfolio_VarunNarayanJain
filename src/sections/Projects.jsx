    ...
    <section className="min-h-screen w-full py-20 px-6 md:px-20 relative z-10 pointer-events-none">
      <div className="max-w-7xl mx-auto pointer-events-auto">
        <motion.span 
          ...
          viewport={{ once: true }}
          className="text-accent font-mono text-sm mb-12 block drop-shadow-md font-medium"
        >
          03. PROJECTS
        </motion.span>
          ...
          {projects.map((project, index) => (
            ...
            >
              <div className="flex justify-between items-start mb-8">
                <h3 className="text-xl font-serif text-white drop-shadow-lg">{project.title}</h3>
                <div className="flex gap-4">
                  <a href={project.github} className="text-gray-200 hover:text-white transition-colors"><Github size={20} /></a>
                  <a href={project.link} className="text-gray-200 hover:text-white transition-colors"><ExternalLink size={20} /></a>
                </div>
              </div>
              <p className="text-gray-100 mb-10 leading-relaxed text-sm font-light">
                {project.description}
              </p>
              <div className="flex flex-wrap gap-3">
                {project.tags.map((tag, i) => (
                  <span key={i} className="text-[11px] font-mono text-accent bg-accent/10 px-3 py-1.5 rounded border border-accent/20 font-medium">
                    {tag}
                  </span>
                ))}
              </div>
